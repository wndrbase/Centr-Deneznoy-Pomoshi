		...
		<ul class="offices__list">
			<?foreach($arResult["ITEMS"] as $key => $arItem):?>
				...
				<li class="offices__item" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
					...
					<?endif;?>
					<?if($arItem['DISPLAY_PROPERTIES']['CDP_G_OFFICE_SCHEDULE']['~VALUE']['FORMAT_TEXT']):?>
						<div class="offices__line">
							<span class="offices__line-label"><?=Loc::getMessage("CDP_BA_SCHEDULE")?></span>
							<span class="offices__line-value"><?=implode('<br>', $arItem['DISPLAY_PROPERTIES']['CDP_G_OFFICE_SCHEDULE']['~VALUE']['FORMAT_TEXT'])?></span>
						</div>
					<?endif;?>
				</li>
			<?endforeach;?>
